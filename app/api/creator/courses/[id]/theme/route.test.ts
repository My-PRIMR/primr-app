...
function req(body: object) {
  return new Request('http://localhost/api/creator/courses/C1/theme', {
    method: 'PATCH',
...
describe('PATCH /api/creator/courses/[id]/theme', () => {
  ...
  it('404 when course missing', async () => {
  ...
  it('403 upgrade_required when theme tier exceeds user plan', async () => {
    ...
    const res = await PATCH(req({ theme: 'enterprise' }), {
      params: Promise.resolve({ id: 'C1' }),
    })
    expect(res.status).toBe(403)
  ...
  it('200 on success', async () => {
    ...
    const body = await res.json()
    expect(body).toEqual({ id: 'C1', theme: 'chalk' })
  })
})
