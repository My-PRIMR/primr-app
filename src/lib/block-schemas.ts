...
const RAW_DEFINITIONS: BlockSchemaDef[] = [
  ...
  {
    type: 'equation-fill-in-the-blank',
    isInteractive: true,
    props: {
      equations: {
        type: 'Array<{ latex: string, label?: string, explanation?: string }>',
        required: true,
        description: 'Each step is a LaTeX equation with \\\\blank{answer} markers where the learner types. The content inside \\\\blank{} is the correct answer as a math expression — e.g. \\\\blank{\\\\sqrt{b^2-4ac}}. Nested braces are supported. Example: "x = \\\\frac{-b \\\\pm \\\\blank{\\\\sqrt{b^2-4ac}}}{\\\\blank{2a}}"',
      },
      context: { type: 'string', description: 'optional markdown (1–3 sentences) explaining what the learner should fill in and how' },
      ...common,
    },
  },
  {
    ...
    isInteractive: false,
    props: {
      functions:      { type: 'Array<{ expression: string, label?: string, color?: string }>', required: true, description: 'expression uses x as variable, supports sin/cos/sqrt/etc. label is the human-readable name shown in the legend (e.g. "Positive slope (m=2)")' },
      xMin:           { type: 'number', default: '-10' },
      xMax:           { type: 'number', default: '10' },
      yMin:           { type: 'number', default: '-10' },
      yMax:           { type: 'number', default: '10' },
      functionLetter: { type: 'string', description: 'letter used to label functions in the tracer tooltip — e.g. "f" → f₁(x), or "g", "h", "p" for different mathematical contexts. Default: "f"' },
      context:        { type: 'string', description: 'optional markdown (1–3 sentences) explaining what this tool does and how the learner should interact with it. Use when the interface needs orientation beyond what the preceding narrative already covers.' },
      ...common,
    },
  },
